--//========================================================
--// TUYEN MOD - ULTRA FIX LAG & NO FOG REWRITE 
--// ADDED BOAT SPEED FEATURE (MAX 430)
--//========================================================
...
local Settings = {
    ...
    ClearFog = false,       -- Xóa sương mù toàn map
...
local PlayerStats = {
    WalkSpeed = 16,
    JumpPower = 50,
    BoatSpeed = 100,        -- Tốc độ tàu mặc định
    EnableSpeed = false,
    EnableJump = false,
    EnableBoatSpeed = false -- Bật/Tắt tăng tốc thuyền
}
...
Close.BackgroundColor3 = Color3.fromRGB(220, 50, 65)
...
-- SLIDER TỐC ĐỘ LÁI TÀU (MAX 430)
CreateSliderCard("🚢 Tốc Độ Lái Tàu (Max 430)", "Tăng tốc độ di chuyển khi điều khiển thuyền", 50, 430, 100, TabSpeed, 138, function(Val)
    PlayerStats.BoatSpeed = Val
    PlayerStats.EnableBoatSpeed = (Val > 50)
end)

--========================================================
-- ENGINE SÀN BĂNG, TỐC ĐỘ LÁI TÀU VÀ XÓA LỖI ĐỎ MÀN HÌNH
--========================================================
...
RunService.RenderStepped:Connect(function()
    pcall(function()
        ...
        if Hum then
            ...
            -- TÍNH NĂNG TĂNG TỐC LÁI TÀU (BOAT SPEED)
            if PlayerStats.EnableBoatSpeed and Hum.SeatPart and Hum.SeatPart:IsA("VehicleSeat") then
                local VehicleSeat = Hum.SeatPart
                VehicleSeat.MaxSpeed = PlayerStats.BoatSpeed
                VehicleSeat.Torque = 100000 -- Tăng lực kéo giúp thuyền tăng tốc tức thì
            end
        end
