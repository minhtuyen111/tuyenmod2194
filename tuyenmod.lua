--//========================================================
--// TUYEN MOD - ULTRA FIX LAG & NO FOG ALL MAP (FIX COLOR)
--// SAFE EXECUTION | DRAGGABLE | 5S INTRO | NO SCREEN RED
--//========================================================
...
-- BẢNG CÀI ĐẶT TÍNH NĂNG
local Settings = {
    FreezeWater = false,    -- Đóng băng biển
    ClearFog = false,       -- Xóa sương mù toàn map (No Fog)
...
--========================================================
-- OPTIMIZATION ENGINE & NO FOG ALL MAP
--========================================================
...
local function ApplyGlobalLighting()
    pcall(function()
        -- FIX XÓA SƯƠNG MÙ TOÀN MAP (NO FOG ALL MAP)
        if Settings.ClearFog then
            Lighting.FogStart = 1e12
            ...
            -- Xóa toàn bộ sương mờ không khí / Atmosphere
            for _, v in ipairs(Lighting:GetChildren()) do
                if v:IsA("Atmosphere") then
                    v.Density = 0
                    v.Offset = 0
                elseif v:IsA("PostEffect") or v:IsA("ColorCorrectionEffect") or v:IsA("BlurEffect") then
                    v.Enabled = false
                end
            end
        else
            ...
            Lighting.GlobalShadows = Settings.Shadows and false or OriginalLighting.GlobalShadows
            Lighting.Ambient = OriginalLighting.Ambient
            Lighting.OutdoorAmbient = OriginalLighting.OutdoorAmbient

            for _, v in ipairs(Lighting:GetChildren()) do
                if v:IsA("Atmosphere") then
                    v.Density = 0.3 -- Mức tiêu chuẩn
                elseif v:IsA("PostEffect") or v:IsA("ColorCorrectionEffect") or v:IsA("BlurEffect") then
                    v.Enabled = not Settings.Particles
                end
            end
        end
...
local TogglesList = {
    {"🧊 Đóng Băng Biển", "Sàn băng xanh an toàn chống dìm biển", "FreezeWater"},
    {"☀️ Xóa Sương Mù All Map", "Nhìn rõ 100% mọi đảo, xóa sương mờ", "ClearFog"},
...
RunService.RenderStepped:Connect(function()
    pcall(function()
        ...
        -- CHỐNG DÌM VÀ TẠO BĂNG KHÔNG ĐỎ MÀN HÌNH
...
CreateInfoLine("☀️ Xóa sương mù all map, làm sáng map & xem xuyên đảo.", 9, COLOR_TEXT_SUB, 12)
CreateInfoLine("🧊 Sàn băng đóng băng biển siêu an toàn, chống dìm.", 9, COLOR_TEXT_SUB, 13)
