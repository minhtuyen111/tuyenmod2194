--//========================================================
--// TUYEN MOD - ULTRA FIX LAG & ANTI-DROWN ICE WATER
--// FONT: BuilderSans | MAX PERFORMANCE & SAFE WATER
--//========================================================
...
--========================================================
-- OPTIMIZATION ENGINE (SIÊU TỐI ƯU CỰC CƯỜNG)
--========================================================
...
local function ProcessObject(Object)
    pcall(function()
        ...
        -- Không can thiệp vào nhân vật chính ngoại trừ Trang phục nếu bật HideAccessories
        ...
        -- TẮT HIỆU ỨNG TẬN GỐC (PARTICLES, BEAMS, TRAILS, VFX, HIGHLIGHTS...)
        if Object:IsA("ParticleEmitter") or Object:IsA("Trail") or Object:IsA("Beam") or Object:IsA("Sparkles") 
        or Object:IsA("Fire") or Object:IsA("Smoke") or Object:IsA("Highlight") or Object:IsA("Explosion") then
        ...
        -- CHUYỂN SANG VẬT THỂ NHẮN (SMOOTH PLASTIC + REFLECTANCE = 0)
        if Object:IsA("BasePart") then
    ...
    pcall(function()
        ...
        if Terrain then
            ...
            if Settings.FreezeWater then
                Terrain:SetMaterialColor(Enum.Material.Water, Color3.fromRGB(150, 220, 255))
                Terrain.WaterColor = Color3.fromRGB(150, 220, 255)
...
local TogglesList = {
    ...
    {"Xóa Sạch Hiệu Ứng", "Triệt hạ toàn bộ lửa, khói, skill VFX", "Particles"},
    ...
    {"Xóa Hoàn Toàn Biển", "Làm nước biển tệp màu vô hình", "RemoveWater"},
...
-- Tự động chạy tối ưu ban đầu
task.defer(RefreshAllObjects)

--========================================================
-- TAB 2: DI CHUYỂN & ĐÓNG BĂNG NƯỚC (FIX CHỐNG DÌM CHẾT)
--========================================================
...
IceLabel.Text = "Đóng Băng Mặt Biển (Chống Dìm)"
...
IceDesc.Text = "Sàn băng dày chống xuyên, tự đẩy lên nếu xài skill lỡ rơi xuống nước"
...
--========================================================
-- ENGINE CHỐNG XUYÊN & CHỐNG DÌM CHẾT NƯỚC BIỂN
--========================================================

local WATER_LEVEL = 0 -- Mức chuẩn mặt nước biển
local IceFloor = Instance.new("Part")
IceFloor.Name = "TuyenMod2194_AntiDrownIceFloor"
IceFloor.Size = Vector3.new(1000, 10, 1000) -- Tạo mặt sàn dày 10 studs chống lọt
...
RunService.Stepped:Connect(function()
    pcall(function()
        ...
        -- 3. XỬ LÝ SÀN BĂNG VÀ TỰ NỔI CHỐNG DÌM
        if Settings.FreezeWater and Root and Hum then
            IceFloor.CanCollide = true
            -- Đặt sàn băng ở ngay bên dưới mặt nước
            IceFloor.CFrame = CFrame.new(Root.Position.X, WATER_LEVEL - 5, Root.Position.Z)

            -- NẾU NHÂN VẬT BỊ DÌM/DÙNG SKILL RỚT XUỐNG DƯỚI NƯỚC:
            if Root.Position.Y < (WATER_LEVEL + 1) or Hum:GetState() == Enum.HumanoidStateType.Swimming then
                -- Teleport tức thì lên bề mặt băng
                Root.CFrame = CFrame.new(Root.Position.X, WATER_LEVEL + 4, Root.Position.Z)
                Root.AssemblyLinearVelocity = Vector3.new(Root.AssemblyLinearVelocity.X, 0, Root.AssemblyLinearVelocity.Z)
                Hum:ChangeState(Enum.HumanoidStateType.Running)
            end
        else
            IceFloor.CanCollide = false
            IceFloor.CFrame = CFrame.new(0, -2000, 0)
        end
    end)
end)

-- VÔ HIỆU HÓA DAME NƯỚC / LAVA
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            for _, v in ipairs(Workspace:GetDescendants()) do
                if v:IsA("BasePart") and (v.Name:lower():find("water") or v.Name:lower():find("lava") or v.Name:lower():find("kill") or v.Name:lower():find("damage")) then
                    if v ~= IceFloor then
                        v.CanTouch = false
                    end
                end
...
Phiên bản cập nhật đặc biệt:
⚡ Xóa sạch toàn bộ VFX Skill, hiệu ứng, khói lửa.
🧊 Cơ chế Đóng Băng Biển nâng cấp chống chìm & chống dame nước.
🚀 Tối ưu FPS tối đa cho máy yếu/trung bình.
...
Cảm ơn bạn đã sử dụng TUYEN MOD. Chúc bạn có trải nghiệm mượt mà!]]
...
CreateSocialCard("YouTube", "Kênh chia sẻ Mod & Script", "▶", INFO.YouTube, 294)
CreateSocialCard("TikTok", "Các video ngắn trải nghiệm", "🎵", INFO.TikTok, 350)
CreateSocialCard("Discord", "Cộng đồng hỗ trợ & giao lưu", "💬", INFO.Discord, 406)